...
namespace IMS.Plugins.EFCore
{
    public class ProductTransactionRepository : IProductTransactionRepository
    {
        ...
        {
            // Here we need to take away the inventories
            // refactor a bit
            ...
            if (prod != null)
            {
                foreach(var pi in prod.ProductInventories)
                {
                    ...
                    pi.Inventory.Quantity -= quantity * pi.InventoryQuantity; // this reminds us that we need to do a validation, so that if we don't have enough inventory, we have to let the user know                    
